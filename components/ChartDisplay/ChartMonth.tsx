...
import { useDataDate, useControlsMonth } from "@components/ChartDisplay/store";
import { format, monthEnd } from "@formkit/tempo";
import {
  RiArrowLeftSLine,
  RiArrowRightSLine,
  RiDatabase2Fill,
...
export function ChartMonth({ color, ...rest }: Props) {
  const colorBar = color ?? "var(--color-neutral-400)";
  const { next, prev } = useControlsMonth();
  const date = useDataDate();
  const end = format(monthEnd(date), "DD/MM/YY", "es");
